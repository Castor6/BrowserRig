...
const unresponsivePageDiagnosis = (options: { readonly ownsPage: boolean; readonly repaired: boolean }): string => [
  ...
  "Retry after the page settles, ask the user to navigate the preserved tab, or explicitly reset the session with `browserrig session reset --session <id>` (this closes relay-owned tabs and only releases adopted tabs).",
].join("")
...
export const defaultPageRepairedWarning = "The session default page stopped answering automation; BrowserRig reconnected and re-resolved the same tab. References to the old page in state are stale."
...
// Playwright can mask Chrome's cross-extension rejection as a context error or locator timeout.
...
export class ExecuteSandbox {
      ...
      Effect.match({
        onFailure: (error): ExecuteResult => {
          ...
          const diagnostic = error instanceof SessionPageRecoveryError
            ...
            : !hasExplicitTargetSelection(options.targetSelection ?? {}) && this.pageProtectedUi && isMaskedProtectedUiFailure(error)
          ...
          const warnings = this.drainWarnings()
          if (diagnostic === protectedExtensionUiDiagnostic) warnings.push(protectedExtensionUiWarning)
          const logCompactionWarning = formatLogCompactionWarning(logSummary)
          if (logCompactionWarning) {
            warnings.push(logCompactionWarning)
          }
  ...
  private async getGlobals(options: ExecuteOptions): Promise<SandboxGlobals> {
    ...
    const handoff = async (message?: string, options?: HandoffCallOptions) => {
      ...
      try {
        await waitForHandoffPageContext({ page: handoffPage, targetId, timeoutMs: handoffPageContextTimeoutMs })
      } catch (error) {
        const kind = runtimeFailureKind(error)
        if (kind !== "context-destroyed" && kind !== "context-missing") throw error
        throw new Error(`Handoff resolved (${handoffMessage}), but the destination execution context did not become available. The tab was kept; run a short follow-up execute so BrowserRig can re-check it.`, { cause: error })
      }
  ...
  private async getSessionPage({ context, targetSelection }: { readonly context: BrowserContext; readonly targetSelection?: ExecuteTargetSelection }): Promise<Page> {
    ...
    if (this.pendingPageTarget) {
      ...
      if (!replacement || this.pendingPageTarget !== pendingTarget) {
