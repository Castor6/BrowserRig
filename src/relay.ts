...
const makeRelay = Effect.fnUntraced(function* (options: {
  ...
  function handleExtensionMessage(socket: WebSocket, raw: string, generation: number, announcedRootTabIds: Set<number>): void {
    ...
    if (extensionMethod === "log") {
      ...
      console[level](`[browserrig extension] ${text}`)
      return
    }
    if (extensionMethod === "ready") {
      ...
      void Promise.all(workers).then((reconciled) => {
        ...
        if (reconciled.every(Boolean) && ![...failedRootReconciliations].some((key) => key.startsWith(`${generation}:`))) {
          for (const target of registry.listRootTargets()) {
  ...
  const routeCdpCommand = Effect.fn("Relay.routeCdpCommand")(function* (socket: WebSocket, message: CdpRequest) {
    ...
    if (!route) {
      return yield* Effect.fail(new Error(rootRoutable
        ? clientBrowserRigSessionId === undefined
  ...
  const finishAttachedTarget = Effect.fnUntraced(function* (
    ...
    if (extensionRpcGeneration === undefined) {
      if (extensionRpc.connected) refreshTabGrouping(tabId)
    } else {
      // Active-tab attachment keeps every presentation RPC bound to the
      // generation that selected the tab; normal/restored targets use the
      // coalescing best-effort worker so cosmetic grouping cannot gate ready.
  ...
  const attachTab = Effect.fnUntraced(function* (options: {
    ...
    return yield* semaphore.withPermit(Effect.gen(function* () {
      ...
      return yield* attachTabUnlocked({ ...options, extensionRpcGeneration: options.extensionRpcGeneration ?? extensionGeneration })
    }))
  })
  ...
  const reconcileAttachedRootUnlocked = Effect.fnUntraced(function* (tabId: number, generation: number) {
    ...
    if (staged?.targetInfo.targetId === targetInfo.targetId) {
      yield* finishAttachedTarget(staged, undefined, generation)
      return
    }
    ...
    yield* attachTabUnlocked({
      ...
      alreadyAttached: true,
      extensionRpcGeneration: generation,
