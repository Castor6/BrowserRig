...
describe("HTTP request schemas", () => {
  ...
  it("shuts down only the matching managed relay instance", async () => {
    ...
    handler = createHttpRequestHandler({
      ...
      registry,
      sessions: new BrowserRigSessions(`http://127.0.0.1:${port}`, undefined, undefined, registry),
      attachActiveTab: () => Effect.fail(new Error("unexpected active-tab attachment")),
    })
