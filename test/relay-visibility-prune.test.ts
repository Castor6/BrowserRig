      ...
      try {
        ...
        yield* Effect.promise(async () => {
          await expect(sendCdp(sessionClient, {
            id: 11,
            method: "Storage.getCookies",
            sessionId: "unknown-browser-alias",
            params: { browserContextId: "session-context" },
          })).rejects.toThrow("Unknown CDP session unknown-browser-alias for Storage.getCookies")
          await expect(sendCdp(sessionClient, {
            id: 12,
            method: "Storage.clearCookies",
            params: { browserContextId: "session-context" },
          })).rejects.toThrow("BrowserRig blocked Storage.clearCookies")
        })
        expect(extension.commands.filter((command) => command.method === "debugger.sendCommand")).toHaveLength(routedCommands.length)

        const rawSecondCreate = yield* Effect.promise(() => sendCdp(rawClient, {
          id: 13,
          method: "Target.createTarget",
          params: { url: "about:blank" },
        }))
        expect(rawSecondCreate.result?.targetId).toBe("target-4")
        extension.commands.length = 0
        yield* Effect.promise(async () => {
          await expect(sendCdp(rawClient, {
            ...
            method: "Storage.getCookies",
            params: { browserContextId: "raw-context" },
          })).rejects.toThrow("Exactly one visible root target is required for Storage.getCookies")
        })
        expect(extension.commands).toEqual([])
      } finally {
        rawClient.close()
