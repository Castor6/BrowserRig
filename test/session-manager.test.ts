...
describe("BrowserRigSessions", () => {
  ...
  it("restores persisted identity and target ownership with reset JavaScript state", async () => {
    ...
    const restored = new BrowserRigSessions("http://127.0.0.1:0", (_id, onDefaultTargetChange) => {
  ...
  it("persists acquired and cleared default targets through the sandbox callback", async () => {
    ...
    const sessions = new BrowserRigSessions("http://127.0.0.1:0", (id, onDefaultTargetChange) => {
  ...
  it.each(["reset", "delete and recreate", "failed adoption"] as const)("ignores retired sandbox callbacks after %s", async (operation) => {
    const persisted: PersistedSession[][] = []
    const sandboxes: FakeSandbox[] = []
    const sessions = new BrowserRigSessions("http://127.0.0.1:0", (_id, onDefaultTargetChange) => {
  ...
  it("wires target changes into the default sandbox factory", async () => {
    const persisted: PersistedSession[][] = []
    const sessions = new BrowserRigSessions("http://127.0.0.1:0", undefined, {
      onSessionsChanged: (entries) => persisted.push([...entries]),
    })
    sessions.restore([{
      id: "alpha",
  ...
  it("rejects every new mutation during drain while keeping reads and resume available", async () => {
    ...
    for (const mutation of mutations) {
      expect(await Effect.runPromise(Effect.flip(mutation))).toMatchObject({ reason: "inactive", message: "BrowserRig sessions are draining" })
    }
  ...
  it("drains retired sandbox cleanup after its bounded caller wait", async () => {
    await Effect.runPromise(Effect.gen(function* () {
      ...
      const sessions = new BrowserRigSessions("http://127.0.0.1:0", () => sandbox, { lifecycleTimeoutMs: 1_000 })
      sessions.createNew("alpha")
  ...
  it("surfaces a durable adoption rollback failure", async () => {
    ...
    const sessions = new BrowserRigSessions("http://127.0.0.1:0", (_id, onDefaultTargetChange) => makeFakeSandbox({
